        ...
        <div className="animate-fadeIn">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <div>
                    ...
                    <p className="text-slate-400 mt-1">Lịch sử và trạng thái các phiếu nhập hàng vào kho</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div className="chart-container border-l-4 border-emerald-500">
                    <p className="text-sm text-slate-400">Đã nhập kho</p>
                                ...
                                <tbody>
                                    {transfers.map(transfer => (
                                        <tr key={transfer.id} className={`border-b border-slate-700/30 hover:bg-slate-700/30 transition-colors ${transfer.status === 'pending' ? 'bg-amber-500/5' : ''}`}>
                                            <td className="py-4 px-6 font-mono text-sm font-medium text-indigo-400">{transfer.transfer_number}</td>
                                            ...
                                            <td className="py-4 px-6 text-sm text-right font-medium text-emerald-400">{new Intl.NumberFormat('vi-VN').format(transfer.total_value)} đ</td>
                ...
                <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-slate-800 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6 m-4 border border-slate-700/50 shadow-2xl shadow-indigo-500/10">
                        <div className="flex justify-between items-start mb-4">
                            <div>
                                <h2 className="text-xl font-bold text-white">📥 Chi tiết phiếu nhập: {selectedTransfer.transfer_number}</h2>
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
                            ...
                            <div>
                                ...
                                <p className="font-medium text-lg text-emerald-400">{selectedTransfer.total_value.toLocaleString()} đ</p>
                            </div>
                                    ...
                                    <table className="w-full text-sm">
                                        ...
                                        <tbody>
                                            {selectedTransfer.items.map(item => (
                                                <tr key={item.id} className="border-t border-slate-700/50">
                                                    ...
                                                    <td className="px-4 py-3 text-right font-medium text-emerald-400">{item.line_total.toLocaleString()}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                        <tfoot className="bg-slate-700/50 font-medium">
                                            <tr>
                                                ...
                                                <td className="px-4 py-3 text-right text-lg text-emerald-400">{selectedTransfer.total_value.toLocaleString()}</td>
                                            </tr>
                                        </tfoot>
                                    </table>
                                </div>
