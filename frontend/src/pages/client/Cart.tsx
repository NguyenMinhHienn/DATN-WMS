...
/**
 * Trang Phiếu nhập tạm - Hiển thị, quản lý hàng hóa và yêu cầu nhập
 */
...
const Cart: React.FC = () => {
    ...
    const removeItem = async (item: CartItemWithId) => {
        if (!window.confirm(`Bạn có chắc muốn xóa "${item.name}" khỏi phiếu nhập?`)) return;
        const key = cartService.getItemKey(item.product_id, item.variant_id);
        setUpdating(prev => new Set(prev).add(key));
        try {
            if (isAuthenticated && item.api_id) {
    ...
    const handlePlaceOrder = async () => {
        ...
        if (!shipping_address.trim()) { alert('Vui lòng nhập địa chỉ / phòng ban'); return; }
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                    <nav className="mb-4">
                        <ol className="flex items-center gap-2 text-sm text-primary-100">
                            ...
                            <li className="text-primary-300">›</li>
                            <li className="text-white font-medium">Phiếu nhập tạm</li>
                        </ol>
                    </nav>
                    <div className="flex items-center justify-between">
                        <div>
                            <h1 className="text-2xl sm:text-3xl font-bold flex items-center gap-3"><span>📋</span> Phiếu nhập tạm</h1>
                            <p className="text-primary-100 mt-1">
                                {totalCount > 0 ? `${totalCount} hàng hóa trong danh sách` : 'Phiếu nhập trống'}
                                {!isAuthenticated && <span className="ml-2 text-yellow-200">(Chưa đăng nhập)</span>}
                            </p>
                        </div>
                        {cart.length > 0 && (
                            <Link to="/products" className="btn bg-white/20 hover:bg-white/30 text-white backdrop-blur-sm">← Tiếp tục thêm hàng</Link>
                        )}
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                {cart.length === 0 ? (
                    <div className="text-center py-16">
                        ...
                        <h2 className="text-2xl font-bold text-slate-800 mb-2">Phiếu nhập tạm đang trống</h2>
                        <p className="text-slate-500 mb-6">Hãy thêm hàng hóa vào danh sách để tạo yêu cầu nhập kho!</p>
                            ...
                            <div className="card sticky top-4 bg-gradient-to-br from-white to-slate-50 border border-slate-200">
                                ...
                                {showCheckout ? (
                                    <div className="pt-4 space-y-3">
                                        <h3 className="font-semibold text-slate-700">📍 Thông tin yêu cầu nhập</h3>
                                        ...
                                        <input type="tel" placeholder="Số điện thoại *" value={checkoutForm.shipping_phone}
                                            ...
                                            className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-primary-500 focus:border-primary-500" />
                                        <textarea placeholder="Phòng ban / Địa chỉ nội bộ *" rows={2} value={checkoutForm.shipping_address}
                                            onChange={e => setCheckoutForm(prev => ({ ...prev, shipping_address: e.target.value }))}
                                            className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-primary-500 focus:border-primary-500" />

                                        <div>
                                            <label className="text-sm font-medium text-slate-600">Loại yêu cầu</label>
                                            <div className="flex gap-3 mt-1">
                                                <label className={`flex-1 flex items-center gap-2 p-3 border rounded-lg cursor-pointer transition-all ${checkoutForm.payment_method === 'COD' ? 'border-primary-500 bg-primary-50' : 'border-slate-200'}`}>
                                                    ...
                                                    <span className="text-sm">🏢 Nhập nội bộ</span>
                                                </label>
                                                <label className={`flex-1 flex items-center gap-2 p-3 border rounded-lg cursor-pointer transition-all ${checkoutForm.payment_method === 'BANKING' ? 'border-primary-500 bg-primary-50' : 'border-slate-200'}`}>
                                                    ...
                                                    <span className="text-sm">🏭 Nhập từ NCC</span>
                                                </label>
                                            </div>
                                        </div>
                                        ...
                                        <button onClick={handlePlaceOrder} disabled={checkoutLoading}
                                            ...
                                            {checkoutLoading ? '⏳ Đang xử lý...' : '✅ Gửi yêu cầu nhập'}
                                        </button>
                                    ...
                                    <div className="pt-4 space-y-3">
                                        ...
                                        <Link to="/products" className="w-full btn btn-secondary py-3 flex items-center justify-center gap-2">← Tiếp tục thêm hàng</Link>
                                    </div>
                                )}
                                ...
                                <div className="mt-4 pt-4 border-t border-slate-200">
                                    <p className="text-xs text-slate-500 flex items-start gap-2">
                                        <span>ℹ️</span>
                                        <span>{isAuthenticated ? 'Phiếu nhập được lưu an toàn.' : 'Đăng nhập để lưu danh sách và gửi yêu cầu.'}</span>
                                    </p>
                                </div>
                            </div>
                        </div>
